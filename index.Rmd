...
ggplot(dat,aes(x=x,y=close,col=type,group=reorder(Name,close),fill=type))+
  geom_line(size=2)+
...
Column {.sidebar}
-----------------------------------------------------------------------
One measure of a 'healthy' stock is the relationship between its market value and trading volume. Trading volume is the number of buys/sells in a day, and thus, if the value and volume are positively related, this means not only that the stock is valuable but that there is high public interest in it. This animation shows the relationship between value and volume for 15 information tech stocks, 15 real estate stocks, and 27 other stocks over time (2012 - 2018). We can see that info tech and real estate are more often along the diagonal (healthy) than other stock types, on average.
...
```{r}
p1=ggplot(acc,aes(x=model,y=error,fill=stock_type))+
...
Bitcoin Timeseries {data-navmenu="BONUS"}
=======================================
...
bitcoin_ts = ts(rev(bitcoin$Close.Last))
dygraph(bitcoin_ts, main = "Bitcoin Value") %>% 
  dyRangeSelector(dateWindow = c(bitcoin$Date[nrow(bitcoin)],
                                 bitcoin$Date[1]))
```
